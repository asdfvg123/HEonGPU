...
#ifndef HEONGPU_RTF_DECRYPTOR_H
#define HEONGPU_RTF_DECRYPTOR_H
...
namespace heongpu
{
     ...
     * decrypted plaintext.
     */
    template <> class HEDecryptor<Scheme::RTF>
    {
      public:
        /**
         ...
         * decryption.
         */
        __host__ HEDecryptor(HEContext<Scheme::RTF>& context,
                             Secretkey<Scheme::RTF>& secret_key);
         ...
         */
        __host__ void
        decrypt(Plaintext<Scheme::RTF>& plaintext,
                Ciphertext<Scheme::RTF>& ciphertext,
                const ExecutionOptions& options = ExecutionOptions())
        {
            input_storage_manager(
                ciphertext,
                [&](Ciphertext<Scheme::RTF>& ciphertext_)
                {
                    output_storage_manager(
                        plaintext,
                        [&](Plaintext<Scheme::RTF>& plaintext_)
                        {
      ...
      private:
        __host__ void decrypt_bfv(Plaintext<Scheme::RTF>& plaintext,
                                  Ciphertext<Scheme::RTF>& ciphertext,
                                  const cudaStream_t stream);

        __host__ void decryptx3_bfv(Plaintext<Scheme::RTF>& plaintext,
                                    Ciphertext<Scheme::RTF>& ciphertext,
                                    const cudaStream_t stream);
        ...
        __host__ void
        partial_decrypt_bfv(Ciphertext<Scheme::RTF>& ciphertext,
                            Secretkey<Scheme::RTF>& sk,
                            Ciphertext<Scheme::RTF>& partial_ciphertext,
                            const cudaStream_t stream);

        __host__ void
        decrypt_fusion_bfv(std::vector<Ciphertext<Scheme::RTF>>& ciphertexts,
                           Plaintext<Scheme::RTF>& plaintext,
                           const cudaStream_t stream);
