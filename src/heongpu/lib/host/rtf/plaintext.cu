...
namespace heongpu
{
    __host__ Plaintext<Scheme::RTF>::Plaintext(HEContext<Scheme::RTF>& context,
    ...
    void Plaintext<Scheme::RTF>::store_in_device(cudaStream_t stream)
    {
    ...
    void Plaintext<Scheme::RTF>::store_in_host(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            if (memory_size() == 0)
    ...
    Data64* Plaintext<Scheme::RTF>::data()
    {
        if (storage_type_ == storage_type::DEVICE)
        {
    ...
    void Plaintext<Scheme::RTF>::save(std::ostream& os) const
    {
    ...
    void Plaintext<Scheme::RTF>::load(std::istream& is)
    {
        if ((!plaintext_generated_))
        {
    ...
    int Plaintext<Scheme::RTF>::memory_size()
    {
        if (storage_type_ == storage_type::DEVICE)
        {
    ...
    void Plaintext<Scheme::RTF>::memory_clear(cudaStream_t stream)
    {
    ...
    void
    Plaintext<Scheme::RTF>::memory_set(DeviceVector<Data64>&& new_device_vector)
    {
        storage_type_ = storage_type::DEVICE;
    ...
    void Plaintext<Scheme::RTF>::copy_to_device(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            // pass
    ...
    void Plaintext<Scheme::RTF>::remove_from_device(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            device_locations_.resize(0, stream);
    ...
    void Plaintext<Scheme::RTF>::remove_from_host()
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            // pass
