...
namespace heongpu
{
    HEContext<Scheme::RTF>::HEContext(const keyswitching_type ks_type,
    ...
    void
    HEContext<Scheme::RTF>::set_poly_modulus_degree(size_t poly_modulus_degree)
    {
    ...
    void HEContext<Scheme::RTF>::set_coeff_modulus_bit_sizes(
    ...
    void HEContext<Scheme::RTF>::set_coeff_modulus_values(
    ...
    void
    HEContext<Scheme::RTF>::set_coeff_modulus_default_values(int P_modulus_size)
    {
        if ((!coeff_modulus_specified_) && (!context_generated_) &&
            (poly_modulus_degree_specified_))
        {
    ...
    void HEContext<Scheme::RTF>::set_plain_modulus(const int plain_modulus)
    {
    ...
    void HEContext<Scheme::RTF>::print_parameters()
    {
    ...
    void HEContext<Scheme::RTF>::save(std::ostream& os) const
    {
    ...
    void HEContext<Scheme::RTF>::load(std::istream& is)
    {
        if ((!context_generated_))
        {
    ...
    Data64
    HEContext<Scheme::RTF>::generate_Q_mod_t(std::vector<Modulus64> primes,
    ...
    std::vector<Data64>
    HEContext<Scheme::RTF>::generate_coeff_div_plain_modulus(
    ...
    std::vector<Data64> HEContext<Scheme::RTF>::generate_base_matrix_q_Bsk(
    ...
    std::vector<Data64>
    HEContext<Scheme::RTF>::generate_base_change_matrix_m_tilde(
    ...
    Data64 HEContext<Scheme::RTF>::generate_inv_prod_q_mod_m_tilde(
        std::vector<Modulus64> primes, Modulus64 mtilda, int size)
    {
    ...
    std::vector<Data64> HEContext<Scheme::RTF>::generate_inv_m_tilde_mod_Bsk(
    ...
    std::vector<Data64> HEContext<Scheme::RTF>::generate_prod_q_mod_Bsk(
        std::vector<Modulus64> primes, std::vector<Modulus64> bsk_mod, int size)
    {
        std::vector<Data64> prod_q_mod_Bsk;
        for (int i = 0; i < bsk_mod.size(); i++)
    ...
    std::vector<Data64> HEContext<Scheme::RTF>::generate_inv_prod_q_mod_Bsk(
        std::vector<Modulus64> primes, std::vector<Modulus64> bsk_mod, int size)
    {
        std::vector<Data64> inv_prod_q_mod_Bsk;
        for (int i = 0; i < bsk_mod.size(); i++)
    ...
    std::vector<Data64> HEContext<Scheme::RTF>::generate_base_matrix_Bsk_q(
        std::vector<Modulus64> primes, std::vector<Modulus64> bsk_mod, int size)
    {
        std::vector<Data64> base_matrix_Bsk_q;
    ...
    std::vector<Data64> HEContext<Scheme::RTF>::generate_base_change_matrix_msk(
    ...
    std::vector<Data64>
    HEContext<Scheme::RTF>::generate_inv_punctured_prod_mod_B_array(
        std::vector<Modulus64> bsk_mod)
    {
        std::vector<Data64> inv_punctured_prod_mod_B_array;
        for (int i = 0; i < bsk_mod.size() - 1; i++)
    ...
    Data64 HEContext<Scheme::RTF>::generate_inv_prod_B_mod_m_sk(
        std::vector<Modulus64> bsk_mod)
    {
    ...
    std::vector<Data64> HEContext<Scheme::RTF>::generate_prod_B_mod_q(
        std::vector<Modulus64> primes, std::vector<Modulus64> bsk_mod, int size)
    {
        std::vector<Data64> prod_B_mod_q;
        for (int i = 0; i < size; i++)
    ...
    std::vector<Modulus64> HEContext<Scheme::RTF>::generate_q_Bsk_merge_modulus(
        std::vector<Modulus64> primes, std::vector<Modulus64> bsk_mod, int size)
    {
        std::vector<Modulus64> q_Bsk_merge_modulus;
        for (int i = 0; i < size; i++)
    ...
    std::vector<Data64> HEContext<Scheme::RTF>::generate_q_Bsk_merge_root(
    ...
    std::vector<Data64>
    HEContext<Scheme::RTF>::generate_Qi_t(std::vector<Modulus64> primes,
                                          Modulus64& plain_mod, int size)
    {
        std::vector<Data64> Qi_t;
        for (int i = 0; i < size; i++)
    ...
    std::vector<Data64>
    HEContext<Scheme::RTF>::generate_Qi_gamma(std::vector<Modulus64> primes,
    ...
    std::vector<Data64>
    HEContext<Scheme::RTF>::generate_Qi_inverse(std::vector<Modulus64> primes,
                                                int size)
    {
        std::vector<Data64> Qi_inverse;
        for (int i = 0; i < size; i++)
    ...
    Data64
    HEContext<Scheme::RTF>::generate_mulq_inv_t(std::vector<Modulus64> primes,
                                                Modulus64& plain_mod, int size)
    {
        Data64 mulq_inv_t = 1;
        for (int i = 0; i < size; i++)
    ...
    Data64 HEContext<Scheme::RTF>::generate_mulq_inv_gamma(
    ...
    Data64 HEContext<Scheme::RTF>::generate_inv_gamma(Modulus64& plain_mod,
                                                      Modulus64& gamma)
    {
    ...
    template class HEContext<Scheme::RTF>;
