...
namespace heongpu
{
    __host__ Publickey<Scheme::RTF>::Publickey(HEContext<Scheme::RTF>& context)
    {
    ...
    Data64* Publickey<Scheme::RTF>::data()
    {
    ...
    void Publickey<Scheme::RTF>::store_in_device(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            // pass
    ...
    void Publickey<Scheme::RTF>::store_in_host(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            if (memory_size() == 0)
    ...
    void Publickey<Scheme::RTF>::save(std::ostream& os) const
    {
        ...
        else
        {
            throw std::runtime_error(
                "Publickey is not generated so can not be serialized!");
        }
    }

    void Publickey<Scheme::RTF>::load(std::istream& is)
    {
        if ((!public_key_generated_))
        {
    ...
    int Publickey<Scheme::RTF>::memory_size()
    {
        if (storage_type_ == storage_type::DEVICE)
        {
    ...
    void Publickey<Scheme::RTF>::memory_clear(cudaStream_t stream)
    {
    ...
    void
    Publickey<Scheme::RTF>::memory_set(DeviceVector<Data64>&& new_device_vector)
    {
        storage_type_ = storage_type::DEVICE;
    ...
    void Publickey<Scheme::RTF>::copy_to_device(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            // pass
    ...
    void Publickey<Scheme::RTF>::remove_from_device(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            device_locations_.resize(0, stream);
    ...
    void Publickey<Scheme::RTF>::remove_from_host()
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            // pass
    ...
    __host__ MultipartyPublickey<Scheme::RTF>::MultipartyPublickey(
        HEContext<Scheme::RTF>& context, RNGSeed seed)
        : Publickey<Scheme::RTF>(context), seed_(seed)
    {
    }
