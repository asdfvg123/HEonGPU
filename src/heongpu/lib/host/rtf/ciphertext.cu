...
{
    __host__
    Ciphertext<Scheme::RTF>::Ciphertext(HEContext<Scheme::RTF>& context,
    ...
    void Ciphertext<Scheme::RTF>::store_in_device(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            // pass
    ...
    void Ciphertext<Scheme::RTF>::store_in_host(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            if (memory_size() == 0)
    ...
    Data64* Ciphertext<Scheme::RTF>::data()
    {
        if (storage_type_ == storage_type::DEVICE)
        {
    ...
    void Ciphertext<Scheme::RTF>::get_data(std::vector<Data64>& cipher,
                                           cudaStream_t stream)
    {
        int cipher_memory_size =
            cipher_size_ * coeff_modulus_count_ * ring_size_;
    ...
    void Ciphertext<Scheme::RTF>::get_data(HostVector<Data64>& cipher,
                                           cudaStream_t stream)
    {
        int cipher_memory_size =
            cipher_size_ * coeff_modulus_count_ * ring_size_;
    ...
    void Ciphertext<Scheme::RTF>::save(std::ostream& os) const
    {
    ...
    void Ciphertext<Scheme::RTF>::load(std::istream& is)
    {
        if ((!ciphertext_generated_))
        {
    ...
    int Ciphertext<Scheme::RTF>::memory_size()
    {
        if (storage_type_ == storage_type::DEVICE)
        {
    ...
    void Ciphertext<Scheme::RTF>::memory_clear(cudaStream_t stream)
    {
    ...
    void Ciphertext<Scheme::RTF>::memory_set(
    ...
    void Ciphertext<Scheme::RTF>::copy_to_device(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            // pass
    ...
    void Ciphertext<Scheme::RTF>::remove_from_device(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            device_locations_.resize(0, stream);
    ...
    void Ciphertext<Scheme::RTF>::remove_from_host()
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            // pass
