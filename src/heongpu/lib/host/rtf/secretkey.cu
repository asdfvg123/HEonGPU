...
namespace heongpu
{
    __host__ Secretkey<Scheme::RTF>::Secretkey(HEContext<Scheme::RTF>& context)
    {
    ...
    __host__
    Secretkey<Scheme::RTF>::Secretkey(const std::vector<int>& secret_key,
    ...
    __host__
    Secretkey<Scheme::RTF>::Secretkey(const HostVector<int>& secret_key,
                                      HEContext<Scheme::RTF>& context,
                                      cudaStream_t stream)
    {
        if (!context.context_generated_)
    ...
    Data64* Secretkey<Scheme::RTF>::data()
    {
    ...
    void Secretkey<Scheme::RTF>::store_in_device(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            // pass
    ...
    void Secretkey<Scheme::RTF>::store_in_host(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            if (memory_size() == 0)
    ...
    void Secretkey<Scheme::RTF>::save(std::ostream& os) const
    {
    ...
    void Secretkey<Scheme::RTF>::load(std::istream& is)
    {
        if ((!secret_key_generated_))
        {
    ...
    int Secretkey<Scheme::RTF>::memory_size()
    {
        if (storage_type_ == storage_type::DEVICE)
        {
    ...
    void Secretkey<Scheme::RTF>::memory_clear(cudaStream_t stream)
    {
    ...
    void
    Secretkey<Scheme::RTF>::memory_set(DeviceVector<Data64>&& new_device_vector)
    {
        storage_type_ = storage_type::DEVICE;
    ...
    void Secretkey<Scheme::RTF>::copy_to_device(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            // pass
    ...
    void Secretkey<Scheme::RTF>::remove_from_device(cudaStream_t stream)
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            device_locations_.resize(0, stream);
    ...
    void Secretkey<Scheme::RTF>::remove_from_host()
    {
        if (storage_type_ == storage_type::DEVICE)
        {
            // pass
